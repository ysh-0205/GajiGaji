...
def Movie_main(request):
    ...
    post_list = Post.objects.filter(Categorytype__name='영화').annotate(avg_rating=Avg('ratings__rating')).order_by(
        '-created_date')
    ...
    try:
        Movie_category = Categorytype.objects.get(name='영화')
        categories = [Movie_category]
    except Categorytype.DoesNotExist:
        categories = []

    return render(request, 'Movie/Movie_main.html', {'page_obj': page_obj, 'categories': categories})

def Movie_detail(request, pk):
    post = Post.objects.filter(Categorytype__name='영화').get(pk=pk)  # 변수에 할당 필수
    ...
    all_posts = Post.objects.filter(Categorytype__name='영화').exclude(pk=pk)
    ...
    if request.method == 'POST':
        ...
        return redirect('Movie_detail', pk=post.pk)
...
def create(request):
    ...
    if request.method == 'POST':
        ...
        if postform.is_valid():
            ...
            return render(request, 'Movie/Movie_create.html', {'postform': postform, 'categories': categories})
    else:
        postform = PostForm()
        return render(request, 'Movie/Movie_Create.html', {'postform': postform, 'categories': categories})
@login_required(login_url='/accounts/google/login/')
...
def update(request, pk):
    ...
    if request.method == "POST":
        ...
        if postform.is_valid():
            ...
            return redirect('Movie_detail', pk=post.pk)  # 수정 후 상세 페이지로 리다이렉트
        else:
            # 폼이 유효하지 않을 경우, 에러 메시지와 함께 다시 렌더링하거나 다른 처리를 할 수 있습니다.
            return render(request, 'Movie/Movie_create.html', {'postform': postform})
    else:
        # GET 요청일 경우, 기존 데이터를 폼에 채워서 보여줍니다.
        postform = PostForm(instance=post)

    return render(request, template_name='Movie/Movie_create.html', context={'postform': postform})
